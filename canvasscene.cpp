...
void CanvasScene::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    if (itemAt(event->scenePos(), QTransform()) != nullptr)
    {
        ...
        QList<QGraphicsItem *> itemList = items(event->scenePos(), Qt::IntersectsItemShape, Qt::DescendingOrder, QTransform());
            ...
            foreach(QGraphicsItem * item, itemList)
            {
                ...
                if (item->type() == Node::Type)
                {
                    ...
                    else if (connectNode1b == nullptr)
                    {
                        ...
                        // parent1 = connectNode1a->parentItem();
                        // while (parent1->parentItem() != nullptr)
                        // parent1 = parent1->parentItem();

                        parent2 = connectNode1b->findRootParent();
                        // parent2 = connectNode1b->parentItem();
                        // while (parent2->parentItem() != nullptr)
                        // parent2 = parent2->parentItem();
...
//                        parent1 = connectNode2a->parentItem();
//                        while (parent1->parentItem() != nullptr)
//                            parent1 = parent1->parentItem();

                        parent2 = connectNode2b->findRootParent();
//                        parent2 = connectNode2b->parentItem();
//                        while (parent2->parentItem() != nullptr)
//                            parent2 = parent2->parentItem();
            ...
            foreach(QGraphicsItem * item, itemList)
            {
                if (item != nullptr || item != 0)
                {
                   ...
                   else if (item->type() == Node::Type)
                    {
                        ...
                        // delete all edges incident to node that will be deleted
        ...
        {
            undo_Node_Pos *  undoPos = new undo_Node_Pos();
...
{
    if (verbose)
        qDebug() << "Mode: " + QString::number(getMode()) << endl;
    if (mDragged  && (getMode() == CanvasView::none || getMode() == CanvasView::edit))
    {
        ...
        {
            mDragged->setPos(mDragged->mapToParent(mDragged->mapFromScene(event->scenePos())));
        }
    }
}
...
{
    if (mDragged && snapToGrid && (getMode() == CanvasView::none || getMode() == CanvasView::edit))
    {
        int x = 0;
        int y = 0;
        if (mDragged->type() == Graph::Type)
        ...
        else if (mDragged->type() == Node::Type)
        {
            ...
            x = round(mDragged->pos().x() / mCellSize.width()) * mCellSize.width();
            y = round(mDragged->pos().y() / mCellSize.height()) * mCellSize.height();
            mDragged->setPos(x , y);
            clearSelection();
        }
    }
...
void CanvasScene::keyReleaseEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_J:
    {
        ...
        {
            if (connectNode1a->parentItem() != nullptr && connectNode2a != nullptr
                    && connectNode1b != nullptr && connectNode2b != nullptr)
            {
                ...
                qreal angle1 = qAtan2(cn1b.ry() - cn1a.ry(), cn1b.rx() - cn1a.rx());
                qreal angle2 = qAtan2(cn2b.ry() - cn2a.ry(), cn2b.rx() - cn2a.rx());
                ...
                if (check)
                {
                    ...
                    {
                        if (i->type() == Node::Type && i != connectNode2a && i != connectNode2b)
                        {
                            Node * node = qgraphicsitem_cast<Node*>(i);
                           // node->setRotation(qRadiansToDegrees(angle));
                            node->setNodeLabel(count);
                            count++;
                        }
                    }
...
//        {
 //           Graph * graph = qgraphicsitem_cast<Graph*>(item);
//            if (modeType == CanvasView::none)
//                graph->setHandlesChildEvents(true);
//            else
//                graph->setHandlesChildEvents(false);
        //}
    }
}
